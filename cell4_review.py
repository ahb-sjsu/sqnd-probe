...
if SKIP_PROCESSING:
        ...
        for bt, pats in ALL_BOND_PATTERNS.get(language, {}).items():
            for p in pats:
                ...
                if match:
                    ...
                    ctx = detect_context(text, language)
                    return bt, ctx['context'], ctx['confidence']
    ...
    if USE_DRIVE_DATA and DRIVE_AVAILABLE:
        try:
            shutil.copy('data/processed/passages.jsonl', f'{DRIVE_FOLDER}/passages.jsonl')
            shutil.copy('data/processed/bonds.jsonl', f'{DRIVE_FOLDER}/bonds.jsonl')
            print(f"  Copied to Drive: {DRIVE_FOLDER}")
        except Exception as e:
