...
class BIPDisentangler(nn.Module):
    ...
    def __init__(
        self, d_model: int = 768, d_bond: int = 128, d_label: int = 64, dropout: float = 0.1
    ):
        super().__init__()
...
class TimeClassifier(nn.Module):
    ...
    def forward(
        self, z: torch.Tensor, reverse_grad: bool = False, lambda_: float = 1.0
    ) -> torch.Tensor:
        """
        Predict time period.
...
class BIPTemporalInvarianceModel(nn.Module):
    ...
    def __init__(self, config: dict):
        ...
        self.time_classifier = TimeClassifier(
            d_input=config.get("d_bond", 128), n_periods=config.get("n_time_periods", 9)
        )

        self.time_classifier_label = TimeClassifier(
            d_input=config.get("d_label", 64), n_periods=config.get("n_time_periods", 9)
        )

        self.hohfeld_classifier = HohfeldianClassifier(
            d_input=config.get("d_bond", 128), n_classes=config.get("n_hohfeld_classes", 4)
        )

        self.bond_classifier = BondClassifier(d_input=config.get("d_bond", 128), n_bond_types=10)
...
class BIPLoss(nn.Module):
    ...
    def forward(
        ...
        kl_label = -0.5 * torch.mean(
            1
            + outputs["label_logvar"]
            - outputs["label_mean"].pow(2)
            - outputs["label_logvar"].exp()
        )
        ...
        if hohfeld_labels is not None:
            losses["hohfeld"] = (
                self.ce_loss(outputs["hohfeld_pred"], hohfeld_labels) * self.lambda_hohfeld
            )
        else:
...
if __name__ == "__main__":
    ...
    # Test input
    texts = ["This is a test passage about saving a life.", "Another passage about obligations."]
